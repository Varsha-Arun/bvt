...
var testSuite = 'LANGUAGETESTS_AMI_v5.6.1';
var testSuiteDesc = 'Github Organization language tests for AMI v5.6.1';
            ...
            amiApiAdapter.getProjects('',
              function (err, prjs) {
                ...
                var u16allProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16all'}
                  )
                );
                var u16cloallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16cloall'}
                  )
                );
                var u16cppallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16cppall'}
                  )
                );
                var u16golallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16golall'}
                  )
                );
                var u16javallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16javall'}
                  )
                );
                var u16nodallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16nodall'}
                  )
                );
                var u16phpallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16phpall'}
                  )
                );
                var u16pytallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16pytall'}
                  )
                );
                var u16ruballProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16ruball'}
                  )
                );
                var u16scaallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16scaall'}
                  )
                );
                var u16Project = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-6-1/u16'}
                  )
                );
