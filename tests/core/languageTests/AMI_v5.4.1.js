...
var testSuite = 'LANGUAGETESTS_AMI_v5.4.1';
var testSuiteDesc = 'Github Organization language tests for AMI v5.4.1';
...
describe(test,
  function () {
            ...
            amiApiAdapter.getProjects('',
              function (err, prjs) {
                ...
                var u16allProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16all'}
                  )
                );
                var u16cloallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16cloall'}
                  )
                );
                var u16cppallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16cppall'}
                  )
                );
                var u16golallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16golall'}
                  )
                );
                var u16javallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16javall'}
                  )
                );
                var u16nodallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16nodall'}
                  )
                );
                var u16phpallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16phpall'}
                  )
                );
                var u16pytallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16pytall'}
                  )
                );
                var u16ruballProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16ruball'}
                  )
                );
                var u16scaallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16scaall'}
                  )
                );
                var u16Project = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v5-4-1/u16'}
                  )
                );
    ...
    it('4. u16 base image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16', subscriptionOrgName: 'ami-v5-4-1'}).id;
    ...
    it('5. u16 cpp image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16cppall', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('6. u16 php image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16phpall', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('7. u16 go image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16golall', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('8. u16 scala image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16scaall', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('9. u16 java image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16javall', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('10. u16 clojure image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16cloall', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('11. u16 nodejs image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16nodall', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('12. u16 python image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16pytall', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('13. u16all service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16all', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('14. u16 ruby image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16ruball', subscriptionOrgName: 'ami-v5-4-1'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
