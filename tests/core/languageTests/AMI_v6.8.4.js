...
var testSuite = 'LANGUAGETESTS_AMI_v6.8.4';
var testSuiteDesc = 'Github Organization language tests for AMI v6.8.4';
...
describe(test,
  function () {
            ...
            amiApiAdapter.getProjects('',
              function (err, prjs) {
                ...
                var u16cloallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16cloall'}
                  )
                );
                var u16cppallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16cppall'}
                  )
                );
                var u16golallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16golall'}
                  )
                );
                var u16javallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16javall'}
                  )
                );
                var u16nodallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16nodall'}
                  )
                );
                var u16phpallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16phpall'}
                  )
                );
                var u16pytallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16pytall'}
                  )
                );
                var u16ruballProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16ruball'}
                  )
                );
                var u16scaallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16scaall'}
                  )
                );
                var u16Project = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/u16'}
                  )
                );
                var aarch64u16Project = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/aarch64_u16'}
                  )
                );
                var aarch64u16allProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/aarch64_u16all'}
                  )
                );
                var aarch64u16cppallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/aarch64_u16cppall'}
                  )
                );
                var aarch64u16javallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/aarch64_u16javall'}
                  )
                );
                var aarch64u16nodallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/aarch64_u16nodall'}
                  )
                );
                var aarch64u16pytallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/aarch64_u16pytall'}
                  )
                );
                var c7Project = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/c7'}
                  )
                );
                var c7allProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/c7all'}
                  )
                );
                var c7cppallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/c7cppall'}
                  )
                );
                var c7javallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/c7javall'}
                  )
                );
                var c7nodallProject = _.first(
                  _.where(projects, {isOrg: true, fullName: 'ami-v6-8-4/c7nodall'}
                  )
                );
    ...
    it('4. u16 base image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16', subscriptionOrgName: 'ami-v6-8-4'}).id;
    ...
    it('5. u16 cpp image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16cppall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
    ...
    it('6. u16 php image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16phpall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('7. u16 go image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16golall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('8. u16 scala image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16scaall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('9. u16 java image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16javall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('10. u16 clojure image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16cloall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('11. u16 nodejs image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16nodall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('12. u16 python image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16pytall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('13. u16all service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16all', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('14. u16 ruby image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'u16ruball', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('15. aarch64u16 service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'aarch64_u16', subscriptionOrgName: 'ami-v6-8-4'}).id;
    ...
    it('16. aarch64u16all service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'aarch64_u16all', subscriptionOrgName: 'ami-v6-8-4'}).id;
        if(global.testManual === 'true') {
          console.log('Skipping as this test case cannot be run as part of automation');
          return done();
        }
    ...
    it('17. aarch64u16cppall service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'aarch64_u16cppall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        if(global.testManual === 'true') {
          console.log('Skipping as this test case cannot be run as part of automation');
          return done();
        }
    ...
    it('18. aarch64u16javall service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'aarch64_u16javall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        if(global.testManual === 'true') {
          console.log('Skipping as this test case cannot be run as part of automation');
          return done();
        }
    ...
    it('19. aarch64u16nodall service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'aarch64_u16nodall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        if(global.testManual === 'true') {
          console.log('Skipping as this test case cannot be run as part of automation');
          return done();
        }
    ...
    it('20. aarch64u16pytall service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'aarch64_u16pytall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        if(global.testManual === 'true') {
          console.log('Skipping as this test case cannot be run as part of automation');
          return done();
        }
    ...
    it('21. c7 service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'c7', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('22. c7all service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'c7all', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('23. c7cppall service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'c7cppall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('24. c7javall service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'c7javall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('25. c7nodall service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'c7nodall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
    ...
    it('26. c7pytall service image project build is successful',
      function (done) {
        var runId = _.findWhere(runs, {projectName: 'c7pytall', subscriptionOrgName: 'ami-v6-8-4'}).id;
        global.getRunByIdStatusWithBackOff(amiApiAdapter, runId,
          successStatusCode, done);
      },
      function(err) {
